...
export default function Home() {
  ...
  useEffect(() => {
    for(let student of students){
      const lastFeeDate = new Date(student.date);
      const dueDate = new Date(lastFeeDate);
      dueDate.setDate(dueDate.getDate() + 30); // assuming fee is due monthly

      const now = new Date();
      if (now > dueDate && !student.alarmSet) {
        // schedule notification
        scheduleFeeNotification(student.name, dueDate).then(id => {
          // mark alarm as set
          setStudents(prev =>
            prev.map(s =>
              s.id === student.id ? { ...s, alarmSet: true } : s
            )
          );
        });
      }
